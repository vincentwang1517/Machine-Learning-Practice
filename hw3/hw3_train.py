...
# 	mean = np.mean(x_all, axis=1);
# 	sigma = np.std(x_all, axis = 1);
# 	for i in len(x_all[0]):
# 		x_all[:,i] = (x_all[:,i] - mean) / sigma;
# 	return x;
...
# history = model.fit(x=x_train, y=y_train, validation_split=0.2, epochs=20, batch_size=200);

batchsize = 256;
hist =  model.fit_generator(datagen.flow(x_train, y_train, batch_size=batchsize), steps_per_epoch=3*len(x_train)//batchsize,  epochs = 400, validation_data=(x_valid, y_valid));
